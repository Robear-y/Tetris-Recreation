...
public class ZPiece extends Tetromino{
	...
	public ZPiece(int x, int y) {
		...
		for(int i=0;i<4;i++) {
			switch(i) {
				case(0):Tile one=new Tile(x,y,4);
						tetromino.add(one);
				case(1):Tile two=new Tile(x,y+30,4);
						tetromino.add(two);
				case(2):Tile three=new Tile(x-30,y,4);
						tetromino.add(three);
				case(3):Tile four=new Tile(x+30,y+30,4);
						tetromino.add(four);
			}
	...
	public void rotate() {
		...
		case(0):newX2=tetromino.get(2).x-60;
				...
				if(checkCollisionRotate(newX2,newY2)==false&&checkCollisionRotate(newX3,newY3)==false) {
					tetromino.get(2).x-=60;
					tetromino.get(2).y+=30;
					tetromino.get(3).y+=30;
					nextRotatePhase++;
				}
				break;
		case(1):newX2=tetromino.get(2).x+60;
				newY2=tetromino.get(2).y-30;
				newX3=tetromino.get(3).x;
				newY3=tetromino.get(3).y-30;
				if(checkCollisionRotate(newX2,newY2)==false&&checkCollisionRotate(newX3,newY3)==false) {
